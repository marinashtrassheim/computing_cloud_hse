...
cd app
docker build -t cr.yandex/crpemh7fg1uk4n6bfgje/flask-app:v1.0.0 .
docker push cr.yandex/crpemh7fg1uk4n6bfgje/flask-app:v1.0.0

# Разворачиваем в Yandex Cloud Kubernetes cluster и подключаемся к нему
yc managed-kubernetes cluster get-credentials --id catgt0goq8t3851457dq --external
...
yc iam key create \
  --service-account-name k8s-registry-sa \
  --output k8s-key.json
